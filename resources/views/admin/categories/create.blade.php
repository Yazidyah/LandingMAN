            ...
            <form class="p-4 md:p-5" action="{{ route('admin.categories.store') }}" method="POST">
                ...
                <div class="grid gap-4 mb-4 grid-cols-1">
                    <div class="col-span-1">
                        ...
                        <input type="text" name="category_name" id="category_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Enter category name" required>
                    </div>
                </div>
                <div class="flex justify-end">
                    <button type="button" onclick="closeCreateModal()" class="bg-red-900 hover:bg-red-500 text-white px-4 py-2 rounded mr-2">Cancel</button>
                    <button type="submit" class="bg-tertiary hover:bg-secondary text-white hover:text-tertiary px-4 py-2 rounded">Create</button>
                </div>
            </form>
        </div>
    </div>
